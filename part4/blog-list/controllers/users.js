...
usersRouter.post('/', async (request, response) => {
    ...
    if (!username) return response.status(406).json({ error: 'username must be provided' })
    if (!password) return response.status(406).json({ error: 'password must be provided' })
